...
def dfs(graph, start):
    ...
    stack = []
    stack.append(start) # แอดตัวแรกเข้าไป
    while len(stack) != 0: # ถ้า stack
     ไม่ว่างให้ทำต่อไป
        front_data = stack[0] # ตัวแรก
        stack.pop(0) # ไม่มีใครเชื่อมก็ดูตัวก่อนหน้า

        for i in range(len(graph.vertices[front_data].neighbors)): # วนตามจำนวนที่อยู่ในกราฟ (A, B, C, D, ...)
            stack.append(graph.vertices[front_data].neighbors[i])
